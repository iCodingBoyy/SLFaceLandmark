...
- (BOOL)openWithConfig:(SLFaceLandmarkConfig *)config error:(NSError *__autoreleasing  _Nullable *)error {
    ...
    BOOL isAuthorized = [SLSDKAuthProvider shared].isAuthorized;
    if (!isAuthorized) {
        if (error != nullptr) {
            NSDictionary *userInfo = @{NSLocalizedDescriptionKey:@"SDK未取得授权，请与客服人员联系"};
            *error = [NSError errorWithDomain:SLFaceLandmarkErrorDomain
                                                 code:SLErrorCodeInvalidAuthorization userInfo:userInfo];
        }
        return NO;
    }
